#import "CRAWKLectureWebViewManager.h"

#import "CRAWKLectureWebView.h"
...
@interface CRAWKLectureWebViewManager () <CRAWKLectureWebViewDelegate>

@end

@implementation CRAWKLectureWebViewManager
{
...
- (UIView *)view
{
  CRAWKLectureWebView *webView = [[CRAWKLectureWebView alloc] initWithProcessPool:[WKProcessPool sharedProcessPool]];
...
{
  [self.bridge.uiManager addUIBlock:^(__unused RCTUIManager *uiManager, NSDictionary<NSNumber *, CRAWKLectureWebView *> *viewRegistry) {
    CRAWKLectureWebView *view = viewRegistry[reactTag];
    if (![view isKindOfClass:[CRAWKLectureWebView class]]) {
      RCTLogError(@"Invalid view returned from registry, expecting CRAWKLectureWebView, got: %@", view);
    } else {
      [view goBack];
    }
  }];
...
- (BOOL)webView:(__unused CRAWKLectureWebView *)webView
